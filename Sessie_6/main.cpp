...
int main( int argc, char** argv )
{
    ...
    CommandLineParser parser(argc, argv,
        ...
        "{ video_faces vf  || (required) path to video }"
    );
    ...
    {
        parser.printMessage();
        cerr << "use parameters: --video_faces=faces.mp4";
        return 0;
    }

    /// Collect data from arguments
    string videopath_faces(parser.get<string>("video_faces"));
    if (videopath_faces.empty())
    {
    ...
    VideoCapture cap(videopath_faces); //Create a VideoCapture object and open the input file
